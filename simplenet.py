...
class AdaptiveNoisePredictor(torch.nn.Module):
    ...
    def __init__(self, input_channels=1536, hidden_dim=256):
        ...
        self.conv1 = torch.nn.Conv2d(
            input_channels, hidden_dim, kernel_size=1, bias=True
        )
...
class SimpleNet(torch.nn.Module):
    ...
    def load(
        ...
        **kwargs,
        ...
        self.embedding_size = (
            ...
            else self.target_embed_dimension
        )
        ...
        if self.pre_proj > 0:
            ...
            self.proj_opt = torch.optim.AdamW(
                self.pre_projection.parameters(), lr * 0.1
            )

        # add noise predictor
        self.noise_predictor = AdaptiveNoisePredictor(
            input_channels=self.target_embed_dimension, hidden_dim=256  # 1536
        )
    ...
    def test(self, training_data, test_data, save_segmentation_images):
        ...
        auroc = metrics.compute_imagewise_retrieval_metrics(scores, anomaly_labels)[
            "auroc"
        ]
    ...
    def _evaluate(
        self, test_data, scores, segmentations, features, labels_gt, masks_gt
    ):
        ...
        if len(masks_gt) > 0:
            ...
            for min_score, max_score in zip(min_scores, max_scores):
                norm_segmentations += (segmentations - min_score) / max(
                    max_score - min_score, 1e-2
                )
        ...
        with tqdm.tqdm(total=self.gan_epochs) as pbar:
            for i_epoch in range(self.gan_epochs):
                ...
                for data_item in input_data:
                    ...
                    sigma_map = self.noise_predictor(
                        true_feats_spatial
                    )  # [batch, 1, 28, 28]
                    ...
                    adaptive_noise = adaptive_noise.permute(0, 2, 3, 1).reshape(
                        -1, adaptive_noise.shape[1]
                    )
                    true_feats_flat = true_feats_spatial.permute(0, 2, 3, 1).reshape(
                        -1, true_feats_spatial.shape[1]
                    )
                ...
                pbar_str += (
                    f" p_true:{round(all_p_true, 3)} p_fake:{round(all_p_fake, 3)}"
                )
                if len(all_p_interp) > 0:
                    pbar_str += (
                        f" p_interp:{round(sum(all_p_interp) / len(input_data), 3)}"
                    )
